...
contract LAN {
    ...
    event loanEnded(uint256 poolId);
    ...
    modifier onlyOwner(uint256 _poolId) {
        require(
            (loans[_poolId].owner == msg.sender) ||
                (loans[_poolId].operator == msg.sender),
            "LAN: not owner"
        );
        _;
    }
    ...
    function bid(
        ...
        require(
            ((loan.whitelisted) &&
                (whitelistedAddresses[_poolId][msg.sender])) ||
                (loan.whitelisted == false),
            "LAN: Not Whitelisted"
        );
   ...
   if (numBids!=0){
    ...
    function repay(uint256 _poolId, uint256 _amount)
        external
        onlyOwner(_poolId)
    {
        ...
        if (
            ...
            NFT.safeTransferFrom(address(this), loans[_poolId].owner, loans[_poolId].nftId);
    ...
    function liquidate(uint256 _poolId) external {
        ...
        require(
            ((latestBid.user == msg.sender) && _ended(_poolId)) ||
                (loan.liquidatable && _liquidate(loan, latestBid, _poolId)),
            "LAN: not latest bidder/notover, or not liquidatable"
        );
        // transfer NFT
        IERC721 NFT = IERC721(loan.collectionAddress);
    ...
    /// @param _poolId pool Id
    function _calculateLoanValue(uint256 _poolId)
        internal
        view
        returns (uint256)
